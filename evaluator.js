(() => {
  ...
  function teamCard(name, team) {
    const currentTeam = GAME()?.getState?.()?.teams?.[name] || {};
    const character = currentTeam.character || 'tactician';
    const gesture = currentTeam.gesture || 'neutral';
    return `<article class="eval-card rank-${team.rank}">
      ...
      <div class="eval-card-art eval-rival-window" aria-hidden="true"><div class="eval-rival-figure"><div class="rivalry-3d eval-3d-preview" data-team="${esc(name)}" data-view="evaluation-card" data-character="${esc(character)}" data-gesture="${esc(gesture)}" data-outcome="idle"></div></div><span class="eval-rival-gesture">✦</span><b>${team.rank === 1 ? 'ON TOP' : 'WATCH ME'}</b><i></i><i></i><i></i></div>
  ...
  function rivalryStage(ranking) {
    ...
    const leftGesture = teams[left]?.gesture || 'neutral', rightGesture = teams[right]?.gesture || 'neutral';
    return `<section class="eval-rivalry-stage" aria-label="Animated 3D manager rivalry window"><div class="rivalry-caption"><span>PHASE 3 · 3D RIVALRY WINDOW</span><b>Face to face</b></div><div class="rivalry-spotlight"></div><div class="rivalry-manager rivalry-manager-left"><div class="rivalry-bubble">YOU CALL THAT A SQUAD?</div><div class="rivalry-laugh">HA!</div><div class="rivalry-3d" data-team="${esc(left)}" data-character="${esc(leftCharacter)}" data-gesture="${esc(leftGesture)}" data-side="left" data-outcome="idle" aria-hidden="true"></div><strong>${esc(left)}</strong></div><div class="rivalry-vs">VS</div><div class="rivalry-manager rivalry-manager-right"><div class="rivalry-bubble">KEEP WATCHING.</div><div class="rivalry-laugh">HA!</div><div class="rivalry-3d" data-team="${esc(right)}" data-character="${esc(rightCharacter)}" data-gesture="${esc(rightGesture)}" data-side="right" data-outcome="idle" aria-hidden="true"></div><strong>${esc(right)}</strong></div></section>`;
  }
